...
let loopStarted=false;/* ================= КАТСКАЧЕНА (3D) ================= */
let csScene,csCam,csRoom,csT=0,csShown=-1,csRunning=false;
function buildCutscene(){
  csScene=new THREE.Scene(); csScene.background=new THREE.Color(0x000000);
  csScene.fog=new THREE.FogExp2(0x000000,.1);
  csCam=new THREE.PerspectiveCamera(60,innerWidth/innerHeight,.05,60);
  csRoom=new THREE.Group(); csScene.add(csRoom);
  const wallM=new THREE.MeshStandardMaterial({map:TEX.wallPaint(),roughness:1,color:0x555548});
  const fl=new THREE.Mesh(new THREE.PlaneGeometry(8,8),new THREE.MeshStandardMaterial({map:TEX.floorTile(),roughness:1,color:0x666658}));
  ...
  for(const [x,z,ry] of [[0,-4,0],[0,4,Math.PI],[-4,0,Math.PI/2]]){
    const w=new THREE.Mesh(new THREE.PlaneGeometry(8,3.4),wallM); w.position.set(x,1.7,z); w.rotation.y=ry; csRoom.add(w); }
  const bm=new THREE.MeshStandardMaterial({color:0x999,metalness:.5,roughness:.5});
  const frame=new THREE.Mesh(new THREE.BoxGeometry(2,.1,.9),bm); frame.position.set(-.5,.4,1.5); frame.rotation.y=.6; csRoom.add(frame);
  const mat=new THREE.Mesh(new THREE.BoxGeometry(1.9,.15,.85),new THREE.MeshStandardMaterial({color:0xb8b2a0}));
  mat.position.set(-.5,.5,1.5); mat.rotation.y=.6; csRoom.add(mat);
  const sk=new THREE.MeshStandardMaterial({color:0xcaa27a,roughness:1});
  const torso=new THREE.Mesh(new THREE.CapsuleGeometry(.2,.6,4,8),sk); torso.position.set(-.5,.62,1.5); torso.rotation.set(0,.6,Math.PI/2); csRoom.add(torso);
  const head=new THREE.Mesh(new THREE.SphereGeometry(.17,10,8),sk); head.position.set(-1.15,.66,1.95); csRoom.add(head);
  const doc=new THREE.Group(); csRoom.add(doc);
  const dm=new THREE.MeshStandardMaterial({color:0xe8e6de,roughness:.9});
  const dt=new THREE.Mesh(new THREE.CapsuleGeometry(.25,.8,4,8),dm); dt.position.y=1; doc.add(dt);
  const dh=new THREE.Mesh(new THREE.SphereGeometry(.18,10,8),new THREE.MeshStandardMaterial({color:0xc8b28f})); dh.position.y=1.7; doc.add(dh);
...
function cutLoop(){
  ...
  csT+=dt;
  const steps=CUTSCENE;
  let idx=-1; for(let i=0;i<steps.length;i++) if(steps[i].t<=csT) idx=i;
  if(idx!==csShown){ csShown=idx;
    const el=$('csText');
    if(idx>=0&&steps[idx].txt){ el.style.opacity=0;
      setTimeout(()=>{ el.textContent=steps[idx].txt; el.style.opacity=1; },400); }
    else el.style.opacity=0;
  }
  const a=csT*.1;
  const r=3.8-Math.min(1.8,csT*.03);
  csCam.position.set(Math.sin(a)*r*.6+1.2,1.9-Math.min(.7,csT*.014),Math.cos(a)*r*.5+.4);
  csCam.lookAt(-.5,.7,1.6);
  const ud=csRoom.userData;
  ud.doc.position.x=1.2-Math.min(.5,csT*.02);
  const pos=ud.rain.geometry.attributes.position;
  for(let i=0;i<pos.count;i++){ let y=pos.getY(i)-dt*4; if(y<1)y=3.4; pos.setY(i,y); }
  pos.needsUpdate=true;
  ud.lamp.intensity=1.4+(Math.random()<.04?-1:0)+Math.sin(csT*17)*.06;
  renderer.render(csScene,csCam);
}
function startCutscene(){
  ...
  $('cineBars').classList.remove('hidden');
  state='cut';
  if(!renderer){ initThree(); clock=new THREE.Clock(); }
  if(!csScene) buildCutscene();
  csT=0; csShown=-1; csRunning=true; cutLoop();
}
function endCutscene(){
  if(state!=='cut') return;
  csRunning=false;
  $('cutscene').classList.add('hidden'); $('cineBars').classList.add('hidden');
  $('hud').classList.remove('hidden');
  state='play'; lockPointer();
  SFX.startAmbience();
  toast('Осмотри палату №4. Где-то здесь должна быть записка...',5);
